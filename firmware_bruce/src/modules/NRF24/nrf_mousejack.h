...
// Web / programmatic helpers
// Run a scan and return targets as a JSON string (returns true on success)
bool mj_scan_web(String &outJson);

// Inject a DuckyScript provided in `script` to the specified `targetIndex` (or by address string if targetIndex==-1)
// Returns true on success (injection scheduled/completed)
