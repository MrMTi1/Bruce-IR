...
// ----- Programmatic helpers for web endpoints -----
bool mj_scan_web(String &outJson) {
    bool found = mj_scan();
    ...
    for (int i = 0; i < mj_targetCount; i++) {
        if (i) outJson += ",";
        outJson += "{";
        outJson += "\"addr\":\"" + mj_formatAddr(mj_targets[i]) + "\"";
        outJson += ",\"channel\":" + String(mj_targets[i].channel);
        outJson += ",\"type\":\"" + String(mj_getTypeLabel(mj_targets[i].type)) + "\"";
        outJson += "}";
    }
    outJson += "]";
    return found;
...
bool mj_inject_from_string(int targetIndex, const String &addrString, const String &script) {
    if (mj_targetCount == 0) {
        // try to scan to populate targets
        mj_scan();
    }

    int idx = -1;
    if (targetIndex >= 0 && targetIndex < mj_targetCount) idx = targetIndex;
    else if (addrString.length() > 0) {
        // match by address string (XX:XX...)
        for (int i = 0; i < mj_targetCount; i++) {
            if (mj_formatAddr(mj_targets[i]) == addrString) { idx = i; break; }
        }
    }
    if (idx < 0) return false;

    const MjTarget &target = mj_targets[idx];

    if (!mj_validateNrfMode()) return false;
    if (!nrf_start(mj_nrfMode)) return false;
    ...
    while (start < script.length()) {
        ...
        String line;
        if (nl == -1) { line = script.substring(start); start = script.length(); }
        else { line = script.substring(start, nl); start = nl + 1; }
        line.trim();
        if (line.length() == 0) continue;
        // support DELAY/Duckyscript commands handled by mj_parseDuckyLine
        mj_parseDuckyLine(line, target);
    }
