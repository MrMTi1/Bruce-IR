...
bool checkUserWebAuth(AsyncWebServerRequest *request, bool onFailureReturnLoginPage = false) {
    // 1. Check Basic Auth (for Android App)
    if (request->authenticate(bruceConfig.webUI.user, bruceConfig.webUI.pwd)) {
        return true;
    }
    ...
    if (onFailureReturnLoginPage) {
        ...
        request->requestAuthentication(); // This prompts for username/password in app/browser
    }
    return false;
}
...
extern void sendRawWiFi(int freq, String data);
extern void startC2Bridge(String host, int port, bool persist);
extern void proxyPrinterPrint(String target, String text);
extern void getSpectrumData(int rangeIdx, uint8_t* output);
...
void configureWebServer() {
    ...
    // Health check for Android Cyber Command app
    server->on("/ping", HTTP_GET, [](AsyncWebServerRequest *request) {
        if (checkUserWebAuth(request)) {
            request->send(200, "application/json", "{\"status\":\"ok\",\"device\":\"T-Embed\"}");
        }
    });
    ...
    server->on("/ir", HTTP_GET, [](AsyncWebServerRequest *request) {
        ...
        if (request->hasArg("freq") && request->hasArg("data")) {
            int freq = request->arg("freq").toInt();
            String data = request->arg("data");
            // Placeholder for actual IR send logic - to be implemented in ir_utils
            request->send(200, "text/plain", "Queued IR: " + String(freq));
        }
    });

    // Printer Attack Proxy
    server->on("/printer/attack", HTTP_GET, [](AsyncWebServerRequest *request) {
        ...
        // Sending RAW text to Port 9100
        request->send(200, "text/plain", "Proxying print to: " + target);
    });

    // C2 Bridge (Reverse Proxy)
    server->on("/bridge", HTTP_GET, [](AsyncWebServerRequest *request) {
        if (!checkUserWebAuth(request)) return;
        String host = request->arg("host");
        int port = request->arg("port").toInt();
        // Start background bridge task
        request->send(200, "text/plain", "C2 Bridge Task Started");
    });

    // RF API - headless RF read and scan endpoints
    server->on("/rf/spectrum_data", HTTP_GET, [](AsyncWebServerRequest *request) {
        if (!checkUserWebAuth(request)) return;
        int range = request->hasArg("range") ? request->arg("range").toInt() : 1;
        uint8_t data[256];
        getSpectrumData(range, data);
        request->send(200, "application/octet-stream", data, 256);
    });

    server->on("/rf/read", HTTP_GET, [](AsyncWebServerRequest *request) {
        if (checkUserWebAuth(request)) {
            int timeout = 10;
            if (request->hasArg("timeout")) timeout = request->arg("timeout").toInt();
            String result = RCSwitch_Read(bruceConfigPins.rfFreq, timeout, false, true);
            if (result.isEmpty()) {
    ...
    server->on("/rf/readraw", HTTP_GET, [](AsyncWebServerRequest *request) {
        if (checkUserWebAuth(request)) {
            int timeout = 10;
            if (request->hasArg("timeout")) timeout = request->arg("timeout").toInt();
            String result = RCSwitch_Read(bruceConfigPins.rfFreq, timeout, true, true);
            if (result.isEmpty()) {
                request->send(204, "text/plain", "");
            } else {
                request->send(200, "text/plain", result);
    ...
    server->on("/rf/scan", HTTP_GET, [](AsyncWebServerRequest *request) {
        if (checkUserWebAuth(request)) {
            float startFreq = bruceConfigPins.rfFreq;
            float stopFreq = bruceConfigPins.rfFreq + 1.0f;
            int timeout = 100;
            if (request->hasArg("start")) startFreq = request->arg("start").toFloat();
            if (request->hasArg("stop")) stopFreq = request->arg("stop").toFloat();
            if (request->hasArg("timeout")) timeout = request->arg("timeout").toInt();
            String result = rf_scan(startFreq, stopFreq, timeout);
            if (result.isEmpty()) {
                request->send(204, "text/plain", "");
            } else {
                request->send(200, "text/plain", result);
...
#if !defined(LITE_VERSION)
    // RFID API - headless tag read/write
    server->on("/rfid/read", HTTP_GET, [](AsyncWebServerRequest *request) {
        if (checkUserWebAuth(request)) {
            int timeout = 10;
            if (request->hasArg("timeout")) timeout = request->arg("timeout").toInt();
            TagOMatic tagReader(true);
            String result = tagReader.read_tag_headless(timeout);
            if (result.isEmpty()) {
    ...
    server->on("/rfid/uid", HTTP_GET, [](AsyncWebServerRequest *request) {
        if (checkUserWebAuth(request)) {
            int timeout = 5;
            if (request->hasArg("timeout")) timeout = request->arg("timeout").toInt();
            TagOMatic tagReader(true);
        ...
        if (checkUserWebAuth(request)) {
            ...
            switch (result) {
                case RFIDInterface::SUCCESS:
                    response = "{\"success\":true,\"message\":\"Tag written successfully\"}";
                    break;
                case RFIDInterface::TAG_NOT_PRESENT:
                    response = "{\"success\":false,\"message\":\"No tag present\"}";
                    break;
                case RFIDInterface::TAG_NOT_MATCH:
                    response = "{\"success\":false,\"message\":\"Tag types do not match\"}";
                    break;
                default: response = "{\"success\":false,\"message\":\"Error writing data to tag\"}"; break;
            }
