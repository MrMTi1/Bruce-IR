...
class MainActivity : AppCompatActivity() {
        ...
        adapter = CommandAdapter(currentFolder.items, null, { action, item, pos ->
            when (action) {
                ...
                CommandAdapter.ActionType.OPEN -> {
                    if (item is IrFolder) { currentFolder = item; refreshList() }
                }
    ...
    private fun setupCyberSection() {
        findViewById<Button>(R.id.cardTvBGone).setOnClickListener { showTvBGoneDialog() }
        findViewById<Button>(R.id.cardAdvanced).setOnClickListener { startActivity(Intent(this, AdvancedActivity::class.java)) }
        findViewById<Button>(R.id.cardMacro).setOnClickListener { showMacroListDialog() }
    }

    private fun setupSystemSection() {
        findViewById<Button>(R.id.cardRemote).setOnClickListener { showRemoteDialog() }
        findViewById<Button>(R.id.cardIntercom).setOnClickListener { startActivity(Intent(this, IntercomActivity::class.java)) }
        findViewById<Button>(R.id.cardNavigator).setOnClickListener { startActivity(Intent(this, NavigatorActivity::class.java)) }
        findViewById<Button>(R.id.cardBluetooth).setOnClickListener { startActivity(Intent(this, BluetoothActivity::class.java)) }
        findViewById<Button>(R.id.cardLux).setOnClickListener { startActivity(Intent(this, LuxActivity::class.java)) }
        
        // Dodatkowy long-click dla pro-userów
        findViewById<Button>(R.id.cardNavigator).setOnLongClickListener { 
            startActivity(Intent(this, RfAnalyzerActivity::class.java))
            true
        }
    }
    ...
    private fun showTvBGoneDialog() {
        ...
        startBtn.setOnClickListener {
            ...
            tvBGoneManager.start({ current, total, name -> text1.post { text1.text = getString(R.string.tvgone_progress, current, total); text2.text = getString(R.string.tvgone_code, name) } }, { text1.post { text1.text = getString(R.string.tvgone_finished); text2.text = ""; startBtn.isEnabled = true } })
        }
            ...
            AlertDialog.Builder(this).setItems(ops) { _, w ->
                when(w) {
                    ...
                    1 -> showCommandPicker(allData) { cmd ->
                        val currentList = tvBGoneManager.getAttackList().toMutableList(); currentList.add(IrCommand(cmd.name, cmd.frequency, cmd.pattern))
                        tvBGoneManager.setAttackList(currentList); Toast.makeText(this, "Added: ${cmd.name}", Toast.LENGTH_SHORT).show()
                    }
                    2 -> { tvBGoneManager.setAttackList(emptyList()); Toast.makeText(this, "Reset to built-in codes", Toast.LENGTH_SHORT).show() }
                }
            }.show()
        }
    }
    ...
    private fun showMacroListDialog() {
        ...
        AlertDialog.Builder(this).setTitle(R.string.macro_select).setItems(names.toTypedArray()) { _, which ->
            if (which == macros.size) showNewMacroDialog() else showMacroDetailsDialog(macros[which])
    ...
    private fun showNewMacroDialog() {
        ...
        AlertDialog.Builder(this).setTitle(R.string.macro_new).setView(input).setPositiveButton(R.string.dialog_ok) { _, _ ->
            val name = input.text.toString()
            if (name.isNotEmpty()) { val newMacro = MacroSet(name); macroManager.updateMacro(newMacro); showMacroDetailsDialog(newMacro) }
    ...
    private fun showMacroExtraMenu(macro: MacroSet) {
        ...
        AlertDialog.Builder(this).setItems(ops) { _, w ->
            val all = macroManager.getAllMacros(); val idx = all.indexOfFirst { it.name == macro.name }
            when(w) {
                0 -> {
                    val input = EditText(this).apply { setText(macro.name) }
                    AlertDialog.Builder(this).setTitle(R.string.macro_rename).setView(input).setPositiveButton("OK") { _, _ -> val oldName = macro.name; macro.name = input.text.toString(); all.removeAll { it.name == oldName }; all.add(macro); macroManager.saveAllMacros(all); showMacroDetailsDialog(macro) }.show()
                }
                1 -> { confirmAction(R.string.confirm_delete_msg) { all.removeAll { it.name == macro.name }; macroManager.saveAllMacros(all); showMacroListDialog() } }
                2 -> { confirmAction(R.string.confirm_title) { macro.commands.clear(); macroManager.updateMacro(macro); showMacroDetailsDialog(macro) } }
                3 -> { if (idx > 0) { Collections.swap(all, idx, idx - 1); macroManager.saveAllMacros(all); showMacroListDialog() } }
    ...
    private fun showMacroItemOptions(macro: MacroSet, index: Int) {
        ...
        AlertDialog.Builder(this).setTitle(macro.commands[index].name).setItems(options) { _, which ->
            when (which) {
    ...
    private fun showMacroConfigDialog(macro: MacroSet) {
        ...
        val etDelay = EditText(this).apply { setText(macro.delayMs.toString()); inputType = android.text.InputType.TYPE_CLASS_NUMBER }
    ...
    private fun showMacroPickerForCommand(cmd: Command) {
        val macros = macroManager.getAllMacros()
        if (macros.isEmpty()) { val m = MacroSet("MACRO 1"); m.commands.add(cmd); macroManager.updateMacro(m); Toast.makeText(this, "Dodano do MACRO 1", Toast.LENGTH_SHORT).show(); return }
        val names = macros.map { it.name }.toTypedArray()
        AlertDialog.Builder(this).setTitle(R.string.btn_add_to_macro).setItems(names) { _, which -> macros[which].commands.add(cmd); macroManager.updateMacro(macros[which]); Toast.makeText(this, getString(R.string.toast_move_success, macros[which].name), Toast.LENGTH_SHORT).show() }.show()
    }

    private fun showCommandPicker(folder: IrFolder, onPicked: (Command) -> Unit) {
        val items = folder.items
        val names = items.map { if (it is IrFolder) "[FOLDER] ${it.name}" else (it as Command).name }.toTypedArray()
        AlertDialog.Builder(this).setTitle("Wybierz z ${folder.name}").setItems(names) { _, which -> val selected = items[which]; if (selected is IrFolder) showCommandPicker(selected, onPicked) else if (selected is Command) onPicked(selected) }.setNegativeButton(R.string.dialog_cancel, null).show()
    }

    private fun loadCustomAttack(uri: Uri) {
        try {
            contentResolver.openInputStream(uri)?.use { inputStream ->
                val fileName = uri.lastPathSegment?.lowercase() ?: ""; val list = mutableListOf<IrCommand>()
                if (fileName.endsWith(".json")) {
                    val folder = BruceUtils.streamParseJson(inputStream.bufferedReader()); val allCmds = mutableListOf<Command>(); extractCommands(folder, allCmds); allCmds.forEach { list.add(IrCommand(it.name, it.frequency, it.pattern)) }
                } else {
                    val cmds = BruceUtils.parseIrContent(inputStream.bufferedReader().readText()); cmds.forEach { list.add(IrCommand(it.name, it.frequency, it.pattern)) }
                }
                if (list.isNotEmpty()) { tvBGoneManager.setAttackList(list); Toast.makeText(this, R.string.toast_signal_sent, Toast.LENGTH_SHORT).show(); showTvBGoneDialog() } else Toast.makeText(this, "No IR codes found", Toast.LENGTH_SHORT).show()
            }
        } catch (e: Exception) { Toast.makeText(this, "Load error: ${e.message}", Toast.LENGTH_SHORT).show() }
    }

    private fun extractCommands(folder: IrFolder, list: MutableList<Command>) {
        folder.items.forEach { if (it is Command) list.add(it) else if (it is IrFolder) extractCommands(it, list) }
    }

    private fun showImportUrlDialog() {
        val input = EditText(this).apply { hint = "https://raw.githubusercontent.com/.../file.ir"; setText("https://raw.githubusercontent.com/MrMTi1/Bruce-IR/master/Ir_codes.json") }
        AlertDialog.Builder(this).setTitle(R.string.import_op_url).setView(input).setPositiveButton(getString(R.string.dialog_ok)) { _, _ -> val url = input.text.toString().trim(); if (url.isNotEmpty()) fetchAndImportIr(url) }.setNegativeButton(R.string.dialog_cancel, null).show()
    }
    ...
    private fun fetchAndImportIr(url: String) {
        ...
        Thread {
            try {
                ...
                connection.connectTimeout = 5000; val content = connection.inputStream.bufferedReader().use { it.readText() }; val fileName = finalUrl.substringAfterLast("/").substringBefore("?").uppercase()
                runOnUiThread {
                    if (finalUrl.endsWith(".json", ignoreCase = true)) {
                        try { allData = BruceUtils.streamParseJson(content.reader()); currentFolder = allData; save { runOnUiThread { refreshList() } } } catch (e: Exception) { processIrContent(content, fileName) }
    ...
    private fun processIrContent(content: String, folderName: String) {
        ...
        if (commands.isNotEmpty()) { downloadedFolder.items.add(IrFolder(folderName, commands.toMutableList() as MutableList<Any>)); save { runOnUiThread { refreshList() } }; Toast.makeText(this, getString(R.string.toast_imported, "DOWNLOADED", folderName), Toast.LENGTH_SHORT).show() }
    }

    private fun performExport(uri: Uri) {
        try { val json = gson.toJson(allData); contentResolver.openOutputStream(uri)?.use { it.write(json.toByteArray()) }; Toast.makeText(this, R.string.toast_export_success, Toast.LENGTH_SHORT).show() } catch (e: Exception) {}
    }

    private fun performFullImport(uri: Uri) {
        try { contentResolver.openInputStream(uri)?.use { inputStream -> allData = BruceUtils.streamParseJson(inputStream.bufferedReader()); currentFolder = allData; save { runOnUiThread { refreshList() } } }; Toast.makeText(this, R.string.toast_import_success, Toast.LENGTH_SHORT).show() } catch (e: Exception) {}
    }
    ...
    private fun showEditCmd(cmd: Command, pos: Int) {
        ...
        val btnIcon = Button(this).apply { text = getString(R.string.btn_change_icon, cmd.iconName ?: "AUTO"); setOnClickListener { val icons = arrayOf("AUTO", "POWER", "VOLUME", "MUTE", "PLAY", "PAUSE", "STOP", "UP", "DOWN", "LEFT", "RIGHT", "STAR", "HEART", "BRIGHTNESS", "CONTRAST", "HOME", "SETTINGS", "INFO", "CAMERA", "MIC", "SEARCH", "SEND", "LOCK", "UNLOCK", "LIGHT", "WIFI", "BATTERY", "AC", "FAN", "TV", "OK", "CANCEL", "PLUS", "MINUS"); AlertDialog.Builder(context).setItems(icons) { _, i -> cmd.iconName = if (i == 0) null else icons[i]; this.text = getString(R.string.btn_change_icon, cmd.iconName ?: "AUTO") }.show() } }
        lay.addView(btnIcon)
        ...
        val btnPin = Button(this).apply { text = getString(R.string.btn_pin); setOnClickListener { pinShortcut(cmd) } }; lay.addView(btnPin)
        val btnMove = Button(this).apply { text = getString(R.string.btn_move); setOnClickListener { showMoveDialog(cmd, pos) } }; lay.addView(btnMove)
        val btnAddMacro = Button(this).apply { text = getString(R.string.btn_add_to_macro); setOnClickListener { showMacroPickerForCommand(cmd) } }; lay.addView(btnAddMacro)
    ...
    private fun showInfoDialog() {
        ...
        tvIrStatus.text = if (hasIr) getString(R.string.ir_status_supported) else getString(R.string.ir_status_not_supported); tvIrStatus.setTextColor(if (hasIr) Color.parseColor("#4CAF50") else Color.parseColor("#F44336"))
        val tvUsbStatus = view.findViewById<TextView>(R.id.tvUsbStatus); val hasUsb = transmitter.isUsbDeviceConnected()
        tvUsbStatus.text = if (hasUsb) getString(R.string.usb_ir_status_connected) else getString(R.string.usb_ir_status_disconnected); tvUsbStatus.setTextColor(if (hasUsb) Color.parseColor("#4CAF50") else Color.parseColor("#F44336"))
        val btnChangeMode = view.findViewById<Button>(R.id.btnChangeMode); btnChangeMode.text = "${getString(R.string.transmitter_mode_title)}: ${transmitter.currentMode.name}"
        ...
        swSound.isChecked = prefs.getBoolean("click_sound", true); swSound.setOnCheckedChangeListener { _, isChecked -> prefs.edit().putBoolean("click_sound", isChecked).apply(); adapter.setSoundEnabled(isChecked) }
    ...
    private fun showRemoteDialog() {
        ...
        val inputUser = EditText(this).apply { hint = "Username"; setText("admin") }; val inputPass = EditText(this).apply { hint = "Password"; setText("bruce") }
        layout.addView(inputUrl); layout.addView(inputUser); layout.addView(inputPass)
        AlertDialog.Builder(this).setTitle(R.string.remote_title).setView(layout).setPositiveButton(R.string.remote_connect) { _, _ -> val intent = Intent(this, BruceRemoteActivity::class.java).apply { putExtra("url", inputUrl.text.toString()); putExtra("user", inputUser.text.toString()); putExtra("pass", inputPass.text.toString()) }; startActivity(intent) }.setNegativeButton(R.string.dialog_cancel, null).show()
    }
}
